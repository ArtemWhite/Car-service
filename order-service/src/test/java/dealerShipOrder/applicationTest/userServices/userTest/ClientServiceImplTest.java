...
import dealerShipOrder.application.services.userService.client.ClientServiceImpl;
...
class ClientServiceImplTest {
    ...
    void setUp() {
        ...
        clientResponse.setId("test-user-id");
    ...
    void shouldGetClientByIdSuccessfully() {
        ...
        ClientResponse result = clientService.getUserById("test-user-id");
    ...
    void shouldThrowExceptionWhenUserIsNotClient() {
        ...
        assertThrows(DomainValidationException.class, () -> {
            clientService.getUserById("test-user-id");
        });
    }
    ...
    void shouldChangePasswordSuccessfully() {
        ...
        when(userRepository.findById("test-user-id")).thenReturn(Optional.of(clientSpy));
    ...
    void shouldGetClientOrdersSuccessfully() {
        ...
        when(order.getType()).thenReturn(dealerShipOrder.domain.models.order.OrderType.IN_STOCK);
        when(order.getStatus()).thenReturn(dealerShipOrder.domain.models.order.OrderStatus.PAID);
    ...
    void shouldGetClientTestDrivesSuccessfully() {
        ...
        when(testDrive.getStatus()).thenReturn(dealerShipOrder.domain.models.testDriveRequest.TestDriveStatus.PENDING);
        when(testDrive.getRequestedTime()).thenReturn(LocalDateTime.now());
        when(testDrive.getCarId()).thenReturn("car123");

        when(userRepository.findById("test-user-id")).thenReturn(Optional.of(client));
        when(testDriveRepository.findByClientId("test-user-id")).thenReturn(List.of(testDrive));
