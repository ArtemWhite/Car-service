...
class CarClientControllerIntegrationTest extends BaseIntegrationTest {
    ...
    void setUp() throws Exception {
        ...
        carId = createCarAndAddToTestDriveFleet();
    }
    ...
    private String createCarAndAddToTestDriveFleet() throws Exception {
        ...
        mockMvc.perform(post("/api/manager/cars/{carId}/test-drive-fleet", id)
    ...
    @Transactional
    @Rollback
    void shouldSendTestDriveRequest() throws Exception {
        String requestedTime = LocalDateTime.now().plusDays(1).format(DateTimeFormatter.ISO_LOCAL_DATE_TIME);

        Map<String, String> requestBody = new HashMap<>();
        requestBody.put("requestedTime", requestedTime);

        mockMvc.perform(post("/api/client/cars/{carId}/test-drive", carId)
                        .header("X-User-Id", clientId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(requestBody)))
                .andExpect(status().isAccepted());  // Ожидаем 202 Accepted, а не 200 OK
    }

    @Test
    @Transactional
    @Rollback
    void shouldFailSendTestDriveRequestInPast() throws Exception {
        String requestedTime = LocalDateTime.now().minusDays(1).format(DateTimeFormatter.ISO_LOCAL_DATE_TIME);
    ...
    @Transactional
    @Rollback
    void shouldGetConfigurationsForModel() throws Exception {
        mockMvc.perform(get("/api/cars/configurations/model/{modelId}", UUID.randomUUID().toString())
                        .header("X-User-Id", clientId))
                .andExpect(status().isOk());
    }
}